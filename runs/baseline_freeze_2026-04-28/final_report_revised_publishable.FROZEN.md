...
| GT8 positive coverage | 10 / 23 audit rows — 43.5% | Partially unresolved; gid=0 promotion blocked by downstream pair-split corruption |
...
### 8.8 1_2_crop: Overhead-View Uploaded-Video Case

1_2_crop properties (`1_2_crop_preprocess_report.json`):

- 3,872 frames, 129.07 s, `920x680`, 30 fps
- bad frames: 2 (`0.05%`, dominant `white_ratio`)

Final output (`retail-shop_1_2_crop_tracks.csv`):
...
| ID | Role | Frame range | Rows | Span |
|---|---|---|---|---|
| 1 | Cashier/staff | 33–3,871 | 3,783 | 3,838 f (127.9 s) |
| 2 | Browsing customer | 338–3,674 | 406 | 3,336 f (111.2 s) |

Observed progression from preserved run logs:

| Configuration snapshot | Positive IDs | Customer IDs | Accepted re-entries | New IDs |
|---|---|---|---|---|
| Baseline (`retail-shop_1_2_crop_pipeline_d376689c.log`) | 6 | 5 | 28 | 11 |
| Final (`retail-shop_1_2_crop_pipeline_2ac00464.log`) | 2 | 1 | 36 | 3 |

Final re-entry stats confirm:

- accepted same-source stitch-trust: 5
- accepted single-candidate spatial: 2

### 8.9 Pipeline Improvements for Overhead Views

Three practical improvements were introduced and validated:
...
- **Single-clip overhead tuning bias:** uploaded-mode thresholds were tuned on limited overhead cases.
